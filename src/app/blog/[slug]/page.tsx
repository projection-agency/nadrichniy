...
const telegram = (
  <svg
    ...
    fill="none"
  >
    <g clipPath="url(#clip0_4368_5002)">
      <path
...
const BlogPostPage = () => {
  ...
  const [categoryLabel, setCategoryLabel] = useState("Всі");
  ...
  useEffect(() => {
    if (!postData?.categories?.[0]) {
      setCategoryLabel("Всі");
      return;
    }
    let alive = true;
    resolveCategoryLabel(postData.categories[0]).then((label) => {
